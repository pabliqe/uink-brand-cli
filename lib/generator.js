...
async function generateOgImage(brandData, outputPath, sourcePath = null, ogOptions = {}) {
  ...
  const fullColor = ogOptions.fullColor || false
  ...
  // Full-color logos get a white card in the left column so their colors show correctly.
  // White/alpha-mask logos render directly over the gradient.
  const logoCardW = leftColWidth - 40
  const logoCardH = 300
  const logoCardX = -logoCardW / 2
  const logoCardY = -logoCardH / 2

  const leftColContent = dataUri
    ? (fullColor
      ? `<rect x="${logoCardX}" y="${logoCardY}" width="${logoCardW}" height="${logoCardH}" rx="24" fill="${brandData.colors.background}"/>
    <image href="${dataUri}" x="-120" y="-120" width="240" height="240" preserveAspectRatio="xMidYMid meet"/>`
...
export async function generateAssets(brandData, outDir, force = false, options = {}) {
  ...
  // When --full-color is set, override logo bg to transparent so icon backgrounds
  // don't paint over the logo's own colors.
  const effectiveLogoBg = fullColor ? 'transparent' : (logoOptions.bg || 'auto')
  const effectiveLogoOptions = { ...logoOptions, bg: effectiveLogoBg }
  ...
  if (!refs.ogImage) {
    const existingOg = findFirstExisting(outDir, ['og-image.png', 'og-image.jpg', 'og-image.webp'])
    if (!force && existingOg) {
      console.log(`   ⊙ ${existingOg} (using existing)`)
      refs.ogImage = existingOg
    } else if (sourceLogo) {
      const ogName = `og-image.${ogFormat}`
      const ogTarget = path.join(outDir, ogName)
      await generateOgImage(brandData, ogTarget, sourceLogo, { ...ogOptions, fullColor })
      refs.ogImage = ogName
    } else {
      const ogName = `og-image.${ogFormat}`
      await generateOgImage(brandData, path.join(outDir, ogName), null, ogOptions)
      refs.ogImage = ogName
    }
  }
  ...
  if (!refs.ogImage) {
    refs.ogImage = findFirstExisting(outDir, ['og-image.png', 'og-image.jpg', 'og-image.webp']) || 'og-image.png'
  }
