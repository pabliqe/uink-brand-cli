...
function resolveOgThemePalette(theme, brandData) {
  ...
  const textDark = brandData.colors.text || '#251f1f'
...
async function generateOgImageBuffer(brandData, source = null, ogOptions = {}) {
  ...
  const fullColor = Boolean(ogOptions.fullColor)
  ...
  const contentTop = sheetY + 72
  const titleStartY = contentTop + titleCapH
  ...
  const dotsLayer = theme.showDots
...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="${leftPad}" y="${titleStartY + (i * titleLineHeight)}" font-size="${titleFontSize}" font-weight="800" fill="${theme.textColor}" letter-spacing="-1.2">${escapeXml(line)}</text>
  `).join('')}
...
export async function generateAssetsInMemory(brandData, options = {}) {
  ...
  // fullColor logos: transparent icon backgrounds so logo colors show through.
  // Otherwise keep a solid canvas (CLI default auto behavior).
