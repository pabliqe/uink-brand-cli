...
export function parseBrandFromJson(brandJson, fallbacks = {}) {
  ...
  const rawSiteTitle = extractAny(brandJson, [
    'brand.siteTitle', 'brand.title', 'brand.siteName', 'brand.sitename'
  ])
  ...
  const secondary = extractCssColorAny(brandJson, [
    `${colorsRoot}.secondary`,
    `${colorsRoot}.brand.secondary`,
    `${colorsRoot}.secondary.DEFAULT`,
    `${colorsRoot}.secondary.default`,
  ])
